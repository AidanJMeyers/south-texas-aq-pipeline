...
| `AM_v037_vs_v040_Audit.ipynb` | Aidan Meyers | **v0.3.7 vs v0.4.0 data-loss audit.** Outer-joins `aq_v0_3_7_epa.pollutant_hourly` (EPA + TCEQ blend) against the v0.4.0 schema per (site × pollutant × year), classifying every cell as Match / Improved / EPA-replaced-by-TCEQ / EPA-only-removed / Out-of-scope-site / New-in-v040 / Decreased-vs-tceq. Outputs an investigation queue of rows where TCEQ-baseline rows appear reduced — the only verdict that means real data loss | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/AidanJMeyers/south-texas-aq-pipeline/blob/main/notebooks/AM_v037_vs_v040_Audit.ipynb) |
| `AM_Lab_Meeting_Followup.ipynb` | Aidan Meyers | **Lab meeting follow-up (2026-06-09).** Closes the to-dos Aidan committed to in the meeting with Manasa: EPA-equivalent gap report, refresh-impact analysis on the new TCEQ raw files (~3.1M net new rows recoverable), per-parameter completeness fix for the >100% NOx mystery, 1hr-vs-24hr cadence-mismatch report, Tukey + 3σ outlier flagging, standardized z-scores per (site × parameter × month × hour) baseline, descriptive statistics, and a loss-recovery action plan | [![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/AidanJMeyers/south-texas-aq-pipeline/blob/main/notebooks/AM_Lab_Meeting_Followup.ipynb) |
